...
Bot PRs show the recorded evidence gate, candidate SHA, active Devin session and specific queue holds in the workbench. Security checks can record intentional 4xx rejection tests alongside a successful functional API request; unexpected errors still fail. See [the bot validation flow and PR #6 recovery record](docs/BOT_PR_VALIDATION.md).
