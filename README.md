...
Open [the dashboard](http://127.0.0.1:8000) and choose **Analytics**. No Superset admin login is needed for embedding. Initial image downloads and Superset metadata migrations can take several minutes. `analytics-superset-init` exiting with code **0** is expected; its job is to provision the datasets and seven charts per dashboard (monthly and rolling).
...
The leading **Total merge hours by work type · calendar month** Superset chart shows summed elapsed PR hours for each exclusive category. Bot-authored PRs are counted only in Bots; a human-authored dependency update belongs to Dependencies. Category totals are also available in the current/baseline comparison and monthly coverage tables. Both charts follow the repository and cohort filters. Superset serves three chart requests concurrently, matching its bounded metadata connection pool; additional requests queue instead of exhausting that pool.

Classification uses explicit title types before broad labels: a `fix:` or `feat:` title stays a fix or feature even if it mentions dependencies. Every PR detail includes a classification reason. Reviewed exceptions are scoped to the exact public PR URL (for example, [Superset #39640](https://github.com/apache/superset/pull/39640) adds semantic-layer access permissions). A title without reliable evidence is flagged **Needs classification**, never silently called Other or assumed to be a feature. Rules live in `backend/app/analytics/classification.py`; regenerate the SQL rule blocks with `PYTHONPATH=backend python -m app.analytics.classification`. Parity tests prevent rule drift. A versioned selection salt invalidates chart caches after taxonomy changes; historical rows are reclassified at read time without rewriting the imported source data.

The **Total hours before merge · calendar month** chart sums `(merged_at - created_at)` for all selected merged PRs, attributed to their UTC merge month. For example, three PRs taking 10, 20 and 30 hours contribute 60 hours. This is cumulative elapsed PR time, not engineering effort or hours saved: concurrent waits overlap. It stays monthly even when other charts use rolling windows. Missing history or invalid merge durations produce a gap, while verified empty months produce zero. The accessible monthly table exposes the same totals below the charts.

Month imports use leased claims, atomic ownership checks, idempotent upserts, persisted pagination, bounded requests per turn, exponential retry and GitHub rate-limit reset headers. Imports pause before exhausting the core API quota, reserving 100 calls for engineering workflows. Search ranges split when GitHub's 1,000-result limit is reached. An oversized single day is held for attention instead of silently truncating; incomplete search pages likewise cannot certify coverage. Authentication/repository failures block the request; eight failed attempts move it to `dead_letter`. An operator can inspect `analytics_months` and, after fixing the cause, reset a blocked/dead-letter row to `state='queued', attempts=0, next_retry=0, lease_until=0, lease_token=NULL`. The page shows loading or attention while this happens. The worker polls requests between hourly sync cycles; an active scheduled sync/enrichment pass can delay a newly requested import. This remains a single-VM deployment, not a high-availability service.

Analytics compares **Fixes, Features, Bots, Dependencies, Documentation, Refactoring, Tests, Build & CI, Performance, Releases, Reverts and Maintenance** across seven calendar months or rolling 7–180 day windows. It measures commits per PR, median merge hours, post-review commit rework and added/removed lines, with per-metric sample coverage. A dotted **21 September 2026** rollout marker separates baseline from later observations. The effort-saved calculator uses editable assumptions and completed Devin work; it never equates waiting time with engineering effort. [Design concepts and metric definitions](docs/design/ANALYTICS.md). Upstream/fork selection, six-month/previous/custom baselines, author/label/branch/work-type filters and tracked-work attribution remain available. Time to merge is elapsed creation-to-merge time grouped by merge date; incomplete/small cohorts remain visible and do not establish a causal Devin improvement. [Metric definitions and SQL](docs/POSTGRES_SUPERSET.md) · [code boundaries and tests](docs/CODE_QUALITY.md).
