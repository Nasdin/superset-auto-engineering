...
if [ ! -f /opt/cognition/backups/last-success ] || [ -n "$(find /opt/cognition/backups/last-success -mmin +1560 -print)" ]; then
  echo 'No successful logical backup in the last 26 hours' >&2
  exit 1
fi
