...
import { latestObservations, type LearningData } from "../learningTypes";
...
export function Learning() {
  ...
  const latest = latestObservations(data?.lessons || []);
  const supplied =
    ...
    <main id="main" className="learning-page">
      ...
      {!data ? (
        ...
        <>
          ...
          <section className="memory-scorecard" aria-label="Learning overview">
            {[
              {
                icon: BookOpen,
                value: latest.length,
              ...
              <div className="memory-list">
                {filtered.map((lesson) => (
                  <button
                    ...
                    aria-pressed={
                      selected?.observation.job_id === lesson.observation.job_id
                    }
                    onClick={() => {
                ...
                {!latest.length && (
                  <div className="memory-empty">
                    ...
                    <p>
                      Real run results will populate this journal. Nothing is
                      inferred from an empty history.
                    </p>
            ...
            <div className="memory-detail-column">
              ...
              <ObservationDetail
                ...
                sourceAvailable={Boolean(
                  data.jobs.find((j) => j.id === selected?.observation.job_id),
                )}
          ...
          <div className="memory-evidence-grid">
            ...
            >
              ...
              <p className="memory-journal-note">
                Excludes stale, unfinished and blocked runs. These outcomes do
                not establish that memory caused improvement.
              </p>
            </section>
          </div>
          <Disclosure
          ...
          >
            <p className="memory-explanation">
              ...
              retrieved by Devin is outside this record. Earlier runs are not
              backfilled.
            </p>
            {data.contexts.map((c) => (
              <article className="memory-dispatch" key={c.job_id}>
                <div>
                  <strong>
                    {data.jobs.find((j) => j.id === c.job_id)?.payload.title ||
                      c.job_id.slice(0, 8)}
                  </strong>
                  <p>
          ...
          >
            <div className="learning-intro">
              ...
              <p>
                ...
                notes and supplied context are recorded separately from agent
                claims. Memories never authorize a merge.
              </p>
            </div>
          </Disclosure>
        </>
